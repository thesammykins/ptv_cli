...
func newGTFSStopsOutput(ctx context.Context, store *gtfs.Store, stops []gtfs.NearbyStopResult, attribution string) stopsOutput {
	output := stopsOutput{Stops: make([]stopsStopOutput, 0, len(stops)), Attribution: normalizedText(attribution), DataSource: "gtfs_static", Freshness: freshnessPtr(currentGTFSFreshness(ctx, store)), Warnings: []string{}}
